...
// 画像をBase64 Data URLに変換（satoriは外部URLを直接処理できないため）
...
export const typedApp = app.openapi(route, async (c) => {
	...
	// Get match history (last 5 matches)
	...
	// Convert to MatchHistoryItem format (last 10)
	...
	// Try to get LoL rank from users table
	...
	// アバター画像をBase64に変換（satoriは外部URLを直接処理できない）
