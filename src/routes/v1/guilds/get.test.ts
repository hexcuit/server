...
describe('GET /v1/guilds/:guildId', () => {
	...
	it('returns guild', async () => {
		...
		const res = await client.v1.guilds[':guildId'].$get({ param: { guildId: ctx.guildId } }, authHeaders)
	...
	it('returns 404 when guild not found', async () => {
		const res = await client.v1.guilds[':guildId'].$get({ param: { guildId: 'nonexistent' } }, authHeaders)
