...
describe('GET /v1/users/:discordId', () => {
	...
	it('returns user without rank', async () => {
		...
		const res = await client.v1.users[':discordId'].$get({ param: { discordId: ctx.discordId } }, authHeaders)
	...
	it('returns user with rank', async () => {
		...
		if (res.ok) {
			const data = await res.json()
			expect(data.discordId).toBe(ctx.discordId)
			expect(data.rank).toEqual({ tier: 'GOLD', division: 'II' })
		}
	})

	it('returns 404 when user not found', async () => {
		const res = await client.v1.users[':discordId'].$get({ param: { discordId: 'nonexistent' } }, authHeaders)
